import type { VercelRequest, VercelResponse } from '@vercel/node'; import { GoogleGenerativeAI } from '@google/generative-ai';

function preprocessHtml(raw: string): string { let html = raw; html = html.replace(/<script[\s\S]*?<\/script>/gi, ""); html = html.replace(/<style[\s\S]*?<\/style>/gi, ""); html = html.replace(/<svg[\s\S]*?<\/svg>/gi, ""); html = html.replace(/<noscript[\s\S]*?<\/noscript>/gi, ""); html = html.replace(//g, ""); html = html.replace(/data:[^"'\s)]+/gi, ""); html = html.replace(/\sstyle="[^"]"/gi, ""); html = html.replace(/\sstyle='[^']'/gi, ""); html = html.replace(/\sclass="[^"]"/gi, ""); html = html.replace(/\sclass='[^']'/gi, ""); html = html.replace(/\sdata-[a-z0-9-]+="[^"]*"/gi, ""); html = html.replace(/\s{2,}/g, " "); html = html.replace(/\n{3,}/g, "\n\n"); return html.trim(); }

interface PreExtractedData { instagramHandles: string[]; whatsappNumbers: string[]; phones: string[]; emails: string[]; metaDescription: string; ogTitle: string; ogImage: string; jsonLd: string[]; addresses: string[]; }

function extractStructuredData(html: string, baseUrl: string): PreExtractedData { const result: PreExtractedData = { instagramHandles: [], whatsappNumbers: [], phones: [], emails: [], metaDescription: "", ogTitle: "", ogImage: "", jsonLd: [], addresses: [], };
...
function extractPrioritySections(html: string): { priority: string; body: string } { const sections: string[] = []; const headerMatch = html.match(/<header[\s\S]*?<\/header>/i); if (headerMatch) sections.push([HEADER]\n${headerMatch[0]});
...
const cleanField = (val: any): string => { if (!val || typeof val !== 'string') return ''; const trimmed = val.trim(); if (trimmed.toLowerCase().includes('not found') || trimmed === '-' || trimmed === '""') return ''; return trimmed; };

export default async function handler(req: VercelRequest, res: VercelResponse) { res.setHeader('Access-Control-Allow-Origin', '*'); res.setHeader('Access-Control-Allow-Methods', 'POST,OPTIONS'); if (req.method === 'OPTIONS') return res.status(200).end(); if (req.method !== 'POST') return res.status(405).json({ error: 'Method Not Allowed' });
...
try {
    ...
    // Cross-validation
    if (structured.instagramHandles.length > 0 && !branding.instagramHandle) branding.instagramHandle = structured.instagramHandles[0];
    if (structured.whatsappNumbers.length > 0 && !branding.whatsappNumber) branding.whatsappNumber = structured.whatsappNumbers[0];
    if (structured.phones.length > 0 && !branding.phone) branding.phone = structured.phones[0];
    if (structured.emails.length > 0 && !branding.email) branding.email = structured.emails[0];
